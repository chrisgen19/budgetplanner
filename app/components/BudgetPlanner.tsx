...
export default function BudgetPlanner() {
  ...
  const resetForm = () => {
    const firstExpenseCat = categories.filter(c => c.type === 'expense').sort((a, b) => a.order - b.order)[0]
    setNewItem({
      type: 'expense',
      name: '',
      amount: '',
      category: firstExpenseCat?.name || '',
      frequency: 'one-time',
      startDate: formatDate(new Date(2026, 0, 1)),
      endDate: '',
      selectedDays: []
              ...
              {categoryBreakdown.incomeBreakdown.length === 0 ? (
                ...
                <div className="space-y-4">
                  {categoryBreakdown.incomeBreakdown.map((item, index) => {
                    const cat = categories.find(c => c.name === item.category && c.type === 'income')
                    const catStyle = cat ? getColorFromName(cat.color) : getColorFromName('emerald')
            ...
            <form onSubmit={handleSaveItem} className="p-6 space-y-5">
              ...
              <div className="flex bg-slate-100 p-1 rounded-xl">
                <button
                  ...
                  onClick={() => {
                    const firstExpenseCat = categories.filter(c => c.type === 'expense').sort((a, b) => a.order - b.order)[0]
                    setNewItem({...newItem, type: 'expense', category: firstExpenseCat?.name || ''})
                  }}
                ...
                <button
                  type="button"
                  onClick={() => {
                    const firstIncomeCat = categories.filter(c => c.type === 'income').sort((a, b) => a.order - b.order)[0]
                    setNewItem({...newItem, type: 'income', category: firstIncomeCat?.name || ''})
                  }}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    {categories
                      .filter(cat => cat.type === newItem.type)
